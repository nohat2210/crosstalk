...
function App() {
  ...
  const [deepgramApiKey, setDeepgramApiKey] = useState(
    (config && config.deepgram && config.deepgram.apiKey) || ""
  );
  const [openaiApiKey, setOpenaiApiKey] = useState(
    (config && config.openai && config.openai.apiKey) || ""
  );
  ...
  const populateVoiceList = () => {
    if ("speechSynthesis" in window) {
      ...
      let googleVoice = speechVoices.find(
  ...
  const generateResponse = async (prompt) => {
    const completion = await openai.current.completions.create({
      ...
      prompt:
        "Complete the dialog, sometimes the user isn't done speaking so please anticipate that. \n" +
        prompt,
      stop: [
        `\nSpeaker${humanSpeaker}:`,
        `\nSpeaker${humanSpeaker + 1}:`,
        `\nSpeaker:${humanSpeaker + 2}`,
        `Speaker:${humanSpeaker}`,
      ],
      max_tokens: 100,
    });
    if (
      completion.choices[0].text.substring(0, 11) === `\nSpeaker${aiSpeaker}:`
    ) {
  ...
  const activateMicrophone = () => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
      ...
      live.on(LiveTranscriptionEvents.Open, () => {
        ...
        setContinuation(
          textToTranscript(`Speaker${aiSpeaker}: ${startPhrase}`, true)
        );
        speak(startPhrase);
            ...
            if (data.is_final && candidate.words.length > 0) {
              ...
              startTransition(() => {
                setTranscript((prevTranscript) => {
                  ...
                  let newPrevTranscript = JSON.parse(
                    JSON.stringify(prevTranscript)
                  );
                  newPrevTranscript.words.push(...candidate.words);
                  return newPrevTranscript;
                });
  ...
  const humanRenderTranscript = (transcript) => {
    ...
    for (let i = 0; i < transcript.words.length; i++) {
      ...
      if (previousSpeaker !== word.speaker) {
        ...
        transcriptSegmentations[transcriptSegmentations.length - 1] +=
    ...
    return transcriptSegmentations.map((segmentation, index) => {
      ...
      return (
        <div className={speakerClass} key={index}>
          <div className="chat-header mx-1">{speaker}</div>
          <div className={bubbleClass}>
            {segmentation.replace(`Speaker${aiSpeaker}:`, "Bot:")}
          </div>
        </div>
      );
    });
  ...
  useEffect(() => {
    if (transcript && !isPending) {
      generateResponse(renderTranscript(transcript)).then((response) =>
        setContinuation(textToTranscript(response))
      );
    }
  ...
  return (
    <div className="App">
      <div className="md:container md:mx-auto">
        <h1 className="text-5xl font-bold mb-4">crosstalk</h1>
        <h2 className="text-1xl font-bold my-2">
          a simple system for 2-way interruptible voice interactions between
          humans and LLMs
          <br />
          🗣️🤖
        </h2>
        <div className="content pb-4">
          <button
            className="btn btn-neutral disabled:pointer-events-none join-item mx-2"
        ...
        <div className="text-input">{humanRenderTranscript(transcript)}</div>
        <div className="text-input opacity-25 animate-pulse" ref={chatbox}>
          {continuation ? (aiSpeakingState ? "speaking:" : "predicting:") : ""}
          {humanRenderTranscript(continuation)}
        </div>
